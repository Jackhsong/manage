...
<body class="easyui-layout">
	...
	<!-- 处理二    end -->
	        ...
	        <!-- 填写财务分摊信息的div begin -->
			<div id="financeShareDiv" class="easyui-dialog" align="center" icon="icon-save"  style="width:600px;height:350px;padding:20px 20px;">
...
function agreeRefund(step){
	...
	var cardInfo = "";
	//if(step == 1) {
	//	if($('#refundReasonId').val() == '') {
	//		$.messager.alert("提示",'请填写退款原因！',"info");
	//		return false;
	//	}
	//}
	...
	if(reg.test(money) || (step == 2)){
		$.messager.confirm('tip','确认同意退款申请？',function(b){
			if(b){
				var tempData;
				if(step == 1) {
