...
<div data-options="region:'center',title:'content'" style="padding:5px;">
	<div id="myCss">
		...
		<a onclick="exportWord()" href="javascript:;" class="easyui-linkbutton" style="width:200px">导出发货单信息</a>
		
		<form action="${rc.contextPath}/order/exportWord" id="exportWord" method="post" >
			<table class="search">
					<input type="hidden" id="number" name="number" value="${detail.number!''}" />
					<input type="hidden" id="receiveName" name="receiveName" value="${detail.receiveName!''}" />
					<input type="hidden" id="createTime" name="createTime" value="${detail.createTime!''}" />
					<input type="hidden" id="sendTime" name="sendTime" value="${detail.sendTime!''}" />
					<input type="hidden" id="receiveMobile" name="receiveMobile" value="${detail.receiveMobile!''}" />
					<input type="hidden" id="address" name="address" value="${detail.address!''}" />
					<input type="hidden" id="remark" name="remark" value="${detail.remark2!''}" />
					<input type="hidden" id="realPrice" name="realPrice" value="${detail.realPrice!''}" />
					<input type="hidden" id="accountPoint" name="accountPoint" value="${detail.accountPoint!''}" />
					<input type="hidden" id="id" name="id" value="${detail.id?c}" />
			</table>
		</form>
                ...
                <tr>
                    ...
                    <td>
                    	<select name="province" id="province">
			 			<#if detail.provinceList?exists && (detail.provinceList?size>0) >
						  <#list  detail.provinceList as bl >
						     <#if detail?exists && detail.provinceId?? &&detail.provinceId == bl.provinceId>
						          <option value="${bl.provinceId?c}" selected="selected">${bl.name}</option>
						     <#else>
                ...
                <tr>
                    ...
                    <td>
                    	<select name="city" id="city">
			 				<#if detail.cityList?exists && (detail.cityList?size>0) >
						  <#list  detail.cityList as bl >
						     <#if detail?exists && detail.cityId?? &&detail.cityId ==bl.cityId>
						          <option value="${bl.cityId?c}" selected="selected">${bl.name}</option>
						     <#else>
						          <option value="${bl.cityId?c}">${bl.name}</option>
						      </#if>
						  </#list>
						 </#if>	
			 		   </select>
                ...
                <tr>
                    ...
                    <td>
                    	<select name="district" id="district">
			 			<#if detail.districtList?exists && (detail.districtList?size>0) >
						  <#list  detail.districtList as bl >
						     <#if detail?exists && detail.districtId?? &&detail.districtId == bl.districtId>
						          <option value="${bl.districtId?c}" selected="selected">${bl.name}</option>
						     <#else>
						          <option value="${bl.districtId?c}">${bl.name}</option>
						      </#if>
						  </#list>
						 </#if>	
			 		   </select>
            ...
            buttons:[{
                ...
                handler:function(){
                    $('#updateOrderQuestion_form').form('submit',{
                        url:"",
                        onSubmit:function(){
                        	var templateId = $('#updateOrderQuestion_form_templateId').val();
