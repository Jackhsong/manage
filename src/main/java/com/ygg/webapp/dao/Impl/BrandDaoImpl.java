...
public class BrandDaoImpl extends BaseDaoImpl implements BrandDao
{
    ...
    public List<BrandEntity> findAllBrandByPara(Map<String, Object> para)
        throws Exception
    {
        List<BrandEntity> resultList = getSqlSession().selectList("BrandMapper.findAllBrandByPara", para);
        if (resultList == null)
        {
    ...
    public List<Map<String, Object>> findRelationBrandCategory(Map<String, Object> para) {
        return getSqlSession().selectList("BrandMapper.findRelationBrandCategory", para);
    }

    @Override
    public List<Integer> findRelationBrandCategoryIdsByBrandId(int brandId) {
        try {
            return getSqlSession().selectList("BrandMapper.findRelationBrandCategoryIdsByBrandId", brandId);
        }catch (Exception e) {
            log.error(e);
            return new ArrayList<>();
        }
