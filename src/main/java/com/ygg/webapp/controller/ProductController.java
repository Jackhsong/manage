...
import com.ygg.webapp.util.HttpRequestUtil;
...
public class ProductController
{
    ...
    {  
        ...
        try
        {
            ...
            {   product.setBrandId(1);product.setSellerId(1);
                productBaseService.saveProductBase(product, mobileDetailsImageList);
            }
            else
            {
                // 修改
                ...
                productBaseService.updateProductBase(product, mobileDetailsImageList, saveType);
            ...
            try
            {
                ...
                HttpRequestUtil.reqPost(UtilProperties.getInstance("config.properties").getProperties("sync_url") + "sync/refresh", nvps);
            }
            catch (Exception ex)
            {
    ...
    @ResponseBody
    public ResultEntity checkCodeAndBarCode(@RequestParam(value = "code", required = true) String code,
        @RequestParam(value = "productBaseId", required = false, defaultValue = "0") int productBaseId)
    {
        try
        {
            List<Integer> ids = productBaseService.checkCodeAndBarCode(code);
