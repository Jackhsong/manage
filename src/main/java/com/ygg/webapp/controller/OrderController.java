...
import com.ygg.webapp.entity.User;
...
import com.ygg.webapp.util.CommonEnum.OrderAppChannelEnum;
import com.ygg.webapp.util.CommonUtil;
import com.ygg.webapp.util.FileUtil;
...
import com.ygg.webapp.util.StringUtils;
import com.ygg.webapp.util.UtilProperties;
import com.ygg.webapp.util.XwpfTUtil;
import com.ygg.webapp.util.ZipUtil;
...
public class OrderController {
	...
	public ModelAndView list(HttpServletRequest request, //
		...
		if (source != 1) {
			mv.addObject("accountId", UtilProperties.getInstance("config.properties").getProperties("admin_account_id"));
		} else {
		...
		List<Map<String, Object>> plmap = productService.findAllProductMap();
		List<Map<String, Object>> productList = new ArrayList<Map<String, Object>>();
		for (Map<String, Object> pe : plmap) {
			Map<String, Object> pmap = new HashMap<String, Object>();
			pmap.put("id", pe.get("id"));
			pmap.put("name", pe.get("name"));
			pmap.put("isDiscount", pe.get("isDiscount"));
	...
	@ResponseBody
	public String jsonOrderInfo(@RequestParam(value = "page", required = false, defaultValue = "1") int page,
	...
	@ControllerLog(description = "订单管理-导出发货单")
	public void exportWord(HttpServletRequest request, HttpServletResponse response, HttpSession session,
			...
			@RequestParam(value = "sendTime", required = false, defaultValue = "") String sendTime,
			@RequestParam(value = "receiveMobile", required = false, defaultValue = "") String receiveMobile,
			@RequestParam(value = "address", required = false, defaultValue = "") String address,
			@RequestParam(value = "remark", required = false, defaultValue = "") String remark,
			@RequestParam(value = "realPrice", required = false, defaultValue = "") String realPrice,
			@RequestParam(value = "accountPoint", required = false, defaultValue = "") String accountPoint,
			@RequestParam(value = "id", required = false, defaultValue = "") int id) throws Exception {

		log.info(number);
		log.info(receiveName);
		...
		try {
			...
			log.info("---orderProductInfo---exportWord---jsonStr:" + jsonStr);

			// 导出列表结果
			URL url = new URL("http://bucket20160601.oss-cn-hangzhou.aliyuncs.com/send.docx");
			...
			params.put("${allSals}", realPrice);
			params.put("${accountPoint}", accountPoint);
			...
			int i = 1;
			Double sum = 0.00,count = 0.00,price = 0.00;
			for (Map<String, String> m : list) {
				...
				while (it.hasNext()) {
					...
					if(key.equals("pShortName")){
						params.put("${Pname"+i+"}", value);
					}
					if(key.equals("price")){
						params.put("${Psale"+i+"}", value);
						price = Double.valueOf(value+"");
					}
					if(key.equals("count")){
						params.put("${Pnum"+i+"}", value);
			...
			params.put("${Pcut}", String.valueOf(sum-Double.valueOf(realPrice)));
			...
			response.setHeader("Content-disposition", "attachment;filename=send.docx");
	...
	public void export(HttpServletRequest request, HttpServletResponse response, HttpSession session,
		...
		try {
			...
			if ("result".equals(exportType)) {
				...
				String[] str = { "下单时间", "用户ID", "付款时间", "付款方式", "发货时间", "客户端类型", "订单编号", "订单状态", "详情", "订单总价", "实付金额",
						"收货人", "收货手机", "商家", "发货地", "物流公司", "运单号" };
				...
				for (int i = 0; i < rows.size(); i++) {
					...
					r.createCell(cellIndex++).setCellValue(currMap.get("orderDetail") + "");
				...
				response.setHeader("Content-disposition",
						"attachment;filename=" + URLEncoder.encode(dowName + ".zip", "utf-8"));
				fOut = response.getOutputStream();
				// 设置缓冲区为1024个字节，即1KB
	...
	public ModelAndView orderSendGoods(HttpServletRequest request) throws Exception {
		ModelAndView mv = new ModelAndView();
	...
	public ModelAndView detail(HttpServletRequest request, @PathVariable("id") int id) throws Exception {
		ModelAndView mv = new ModelAndView();
	...
	public String orderProductInfo(HttpServletRequest request, @RequestParam(value = "id", required = true) int id)
			throws Exception {
		String jsonStr = orderService.orderProductJsonStr(id);
		log.info("---orderProductInfo---jsonStr:" + jsonStr);
		return jsonStr;
	}
	...
	@ResponseBody
	public String jsonCompanyCode(HttpServletRequest request) throws Exception {
		...
		for (KdCompanyEnum companyEnum : KdCompanyEnum.values()) {
			...
			map.put("code", companyEnum.getKdnCode());
	...
	public String sendOrder(HttpServletRequest request, @RequestParam(value = "orderId", required = true) int orderId, // 订单ID
			@RequestParam(value = "sendType", required = false, defaultValue = "1") int sendType, // 是否有物流																									// 1：有，0：没有
		...
		number = number.trim();
		if ((sendType == 1)
				&& ("".equals(channel) || "".equals(number) || !StringUtils.isOnlyLettersAndNumber(number))) {
		...
		int resultStaus = 0;
		channel = KdCompanyEnum.getCompanyNameByKdnCode(channel);
	...
	@ResponseBody
	public String kd100Callback(HttpServletRequest request) throws Exception {
	...
	public String batchSendOrder(HttpServletRequest request, @RequestParam("orderFile") MultipartFile file,
			@RequestParam(value = "importType", required = false, defaultValue = "0") int importType) throws Exception {
		try {
			...
			if (startNum == lastNum) {// 可过滤第一行，因为第一行是标题
				map.put("status", 0);
				map.put("msg", "文件为空请确认！");
	...
	@ResponseBody
	public String receiveResultNum(HttpServletRequest request,
			@RequestParam(value = "delete", required = false, defaultValue = "1") int delete) throws Exception {
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("okNum", 0);
		map.put("failNum", 0);
		HttpSession session = request.getSession();
	...
	public void exportSendResult(HttpServletRequest request, HttpServletResponse response) throws Exception {
		response.setContentType("application/vnd.ms-excel");
	...
	public void exportSendTemplate(HttpServletRequest request, HttpServletResponse response) throws Exception {
		response.setContentType("application/vnd.ms-excel");
	...
	public ModelAndView updateTradeStatus(HttpServletRequest request) throws Exception {
		ModelAndView mv = new ModelAndView();
	...
	public String searchOrder(HttpServletRequest request,
			@RequestParam(value = "number", required = false, defaultValue = "0") String number) throws Exception {
		Map<String, Object> map = new HashMap<String, Object>();
	...
	public String updateStatus(HttpServletRequest request,
			...
			@RequestParam(value = "oldStatus", required = true) int oldStatus,
			@RequestParam(value = "newStatus", required = true) int newStatus) throws Exception {
		try {
	...
	@ResponseBody
	public String findOrderOperation(HttpServletRequest request,
	...
	public String updateMarks(HttpServletRequest request, @RequestParam(value = "id", required = true) int id, //
		...
		if (type == 1) {
			...
			para.put("remark2", "客服备注："+remark + "在" + DateTime.now().toString(" yyyy-MM-dd HH:mm:ss") + "&nbsp;&nbsp;&nbsp;");
		}
		try {
			Map<String, Object> map = new HashMap<String, Object>();
	...
	public String updateAddress(HttpServletRequest request, ReceiveAddressEntity address, int tradeId)
		...
		try {
			if (address.getProvince() == null || address.getCity() == null || address.getDistrict() == null
					|| address.getFullName() == null || address.getMobileNumber() == null
					|| address.getDetailAddress() == null || "0".equals(address.getProvince())
					|| "0".equals(address.getCity()) || "0".equals(address.getDistrict())
					|| "".equals(address.getFullName()) || "".equals(address.getMobileNumber())
					|| "".equals(address.getDetailAddress())) {
				Map<String, Object> map = new HashMap<String, Object>();
				map.put("status", 0);
				map.put("msg", "请写完整信息");
				return JSON.toJSONString(map);
	...
	public ModelAndView unSendGoodsList(HttpServletRequest request) throws Exception {
		ModelAndView mv = new ModelAndView();
	...
	@ResponseBody
	public String jsonhbOrderList(HttpServletRequest request,
	...
	public void exportUnSendGoods(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			for (int i = 0; i < result.size(); i++) {
				...
				r.createCell(3).setCellValue(
						currMap.get("countIds_24_to_48") == null ? "" : currMap.get("countIds_24_to_48") + "");
				r.createCell(4).setCellValue(
						currMap.get("sumTotal_24_to_48") == null ? "" : currMap.get("sumTotal_24_to_48") + "");
				r.createCell(5).setCellValue(
						currMap.get("countIds_upper_48") == null ? "" : currMap.get("countIds_upper_48") + "");
				r.createCell(6).setCellValue(
						currMap.get("sumTotal_upper_48") == null ? "" : currMap.get("sumTotal_upper_48") + "");
			}
			fOut = response.getOutputStream();
			workbook.write(fOut);
			fOut.flush();
	...
	public ModelAndView unSendGoodsListDetail(HttpServletRequest request) throws Exception {
		ModelAndView mv = new ModelAndView();
		...
		if (id != null) {
			mv.addObject("id", id);
			// SellerEntity seller =
			// sellerService.findSellerById(Integer.valueOf(id));
			mv.addObject("seller", "");
		} else {
			mv.addObject("id", "0");
		}
		...
		{
			String beginTime = DateTime.now().withHourOfDay(0).withMinuteOfHour(0).withSecondOfMinute(0)
					.toString("yyyy-MM-dd HH:mm:ss");
			String endTime = DateTime.now().withHourOfDay(15).withMinuteOfHour(0).withSecondOfMinute(0)
					.toString("yyyy-MM-dd HH:mm:ss");
	...
	@ResponseBody
	public String jsonUnSendGoodsListDetail(HttpServletRequest request,
			@RequestParam(value = "page", required = false, defaultValue = "1") int page,
			...
			@RequestParam(value = "operType", required = false, defaultValue = "2") String operType) throws Exception {
		Map<String, Object> result = new HashMap<String, Object>();
		if (type == 1 || fromId != 0) {
			if (fromId != 0) {
				sellerId = fromId;
			...
			if ("1".equals(operType)) {
				para.put("payTimeBegin", DateTime.now().withHourOfDay(0).withMinuteOfHour(0).withSecondOfMinute(0)
						.toString("yyyy-MM-dd HH:mm:ss"));
				para.put("payTimeEnd", DateTime.now().withHourOfDay(15).withMinuteOfHour(0).withSecondOfMinute(0)
						.toString("yyyy-MM-dd HH:mm:ss"));
	...
	public void exportUnSendGoodsListDetail(HttpServletRequest request, HttpServletResponse response,
			...
			@RequestParam(value = "sellerType", required = false, defaultValue = "0") int sellerType,
			@RequestParam(value = "sellerId", required = false, defaultValue = "-1") int sellerId,
			@RequestParam(value = "operType", required = false, defaultValue = "2") int operType) throws Exception {
		Map<String, Object> para = new HashMap<String, Object>();
		...
		if (operType == 1) // 当天15小时
		{
			...
			para.put("startTimeBegin", DateTime.now().withHourOfDay(0).withMinuteOfHour(0).withSecondOfMinute(0)
					.toString("yyyy-MM-dd HH:mm:ss"));
			para.put("startTimeEnd", DateTime.now().withHourOfDay(15).withMinuteOfHour(0).withSecondOfMinute(0)
					.toString("yyyy-MM-dd HH:mm:ss"));
		...
		if ("result".equals(exportType)) {// 导出结果
			response.setContentType("application/vnd.ms-excel");
	...
	public ModelAndView sourceList(HttpServletRequest request) throws Exception {
	...
	@ResponseBody
	public String otherSourceInfo(HttpServletRequest request,
			@RequestParam(value = "page", required = false, defaultValue = "1") int page,
			@RequestParam(value = "rows", required = false, defaultValue = "50") int rows,
			@RequestParam(value = "mark", required = false, defaultValue = "") String mark, // url后缀
	...
	public String saveChannel(HttpServletRequest request, @RequestParam(value = "name", required = true) String name,
			@RequestParam(value = "responsibilityPerson", required = true) String responsibilityPerson,
			@RequestParam(value = "id", required = false, defaultValue = "0") int id) throws Exception {
	...
	public String getFakeOrderJsonInfo(@RequestParam(value = "page", required = false, defaultValue = "1") int page,
			...
			@RequestParam(value = "sellerId", required = false, defaultValue = "-1") int sellerId,
			@RequestParam(value = "sellerType", required = false, defaultValue = "0") int sellerType,
			@RequestParam(value = "orderCount", required = false, defaultValue = "0") int orderCount,
			@RequestParam(value = "days", required = false, defaultValue = "0") int days) throws Exception {
		Map<String, Object> para = new HashMap<String, Object>();
		if (page == 0) {
			page = 1;
		}
		...
		if (sellerType != 0) {
			if (sellerType == SellerEnum.SellerTypeEnum.HONG_KONG.getCode()) {
				 ...
				 * para.put("isNeedIdcardImage", 0);
				 */
			}
			// 发货类型为：香港（身份证照片）暂时不用
			/*
			 * else if (sellerType ==
			 * SellerEnum.SellerTypeEnum.HONG_KONG_1.getCode()) {
			 * para.put("sellerType",
			 * SellerEnum.SellerTypeEnum.HONG_KONG_1.getCode());
			 * para.put("isNeedIdcardNumber", 1); para.put("isNeedIdcardImage",
			 * 1); }
			 */
			else {
				para.put("sellerType", sellerType);
	...
	public String importOrderSettlemet(HttpServletRequest request, @RequestParam("orderFile") MultipartFile file,
			...
			@RequestParam(value = "isRight", required = false, defaultValue = "0") int confirm) throws Exception {
		try {
	...
	@ResponseBody
	public String orderSettlementResult(HttpServletRequest request,
			@RequestParam(value = "delete", required = false, defaultValue = "1") int delete) throws Exception {
		List<Map<String, Object>> rList = new ArrayList<Map<String, Object>>();
		HttpSession session = request.getSession();
		Object iList = session.getAttribute("testOrderSettlementList");
	...
	public String calOrderRefundInfo(HttpServletRequest request, //
		...
		Map<String, Object> result = orderService.calOrderRefundInfoByOrderNumber(number, selectProductCount,
				orderProductId, 1);
		return JSON.toJSONString(result);
	}
	...
	public ModelAndView orderFreezeList(HttpServletRequest request) throws Exception {
		ModelAndView mv = new ModelAndView();
	...
	public String jsonOrderFreezeInfo(HttpServletRequest request, //
			@RequestParam(value = "page", required = false, defaultValue = "1") int page, //
			@RequestParam(value = "rows", required = false, defaultValue = "50") int rows, //
			@RequestParam(value = "status", required = false, defaultValue = "-1") byte status) throws Exception {
		Map<String, Object> para = new HashMap<String, Object>();
		if (page == 0) {
			page = 1;
		}
	...
	public ModelAndView sellerSendGoods(HttpServletRequest request) throws Exception {
		ModelAndView mv = new ModelAndView();
		...
		String startTimeEnd = "";
		if (hour < 15) {
			startTimeBegin = DateTime.now().plusDays(-1).withTimeAtStartOfDay().plusHours(15)
					.toString("yyyy-MM-dd HH:mm:ss");
	...
	public void exportSellerSendGoods(HttpServletRequest request, HttpServletResponse response, HttpSession session,
			@RequestParam(value = "timeType", required = false, defaultValue = "0") int timeType, // 订单时间类型，1：最近一个时间段，2：自定义时间
			@RequestParam(value = "startTime1", required = false, defaultValue = "") String startTime1,
			@RequestParam(value = "endTime1", required = false, defaultValue = "") String endTime1,
			@RequestParam(value = "startTime2", required = false, defaultValue = "") String startTime2,
			@RequestParam(value = "endTime2", required = false, defaultValue = "") String endTime2) throws Exception {
		Map<String, Object> para = new HashMap<String, Object>();
	...
	public String jsonOrderSendTimeAnalyzeDetail(//
		...
		try {
			...
			log.error("异步加载每日订单发货时效：payTime=" + payTime + ",beginHour=" + beginHour + ",endHour=" + endHour + ",出错了",
					e);
	...
	public void exportOrderSendTimeAnalyzeDetail(HttpServletRequest request, //
		...
		try {
			...
			String[] str = { "下单时间", "用户ID", "付款时间", "付款方式", "发货时间", "订单来源", "订单编号", "订单状态", "商家备注", "客服备注", "订单总价",
					"实付金额", "收货人", "收货手机", "商家", "发货地", "是否导出", "物流公司", "运单号" };
			Row row = sheet.createRow(0);
			for (int i = 0; i < str.length; i++) {
				Cell cell = row.createCell(i);
				cell.setCellValue(str[i]);
	...
	public String jsonSellerSendTimeAnalyzeDetail(//
		...
		try {
			...
			log.error("异步加载商家发货时效明细：payTime=" + payTime + ",beginHour=" + beginHour + ",endHour=" + endHour + ",出错了",
					e);
			resultMap.put("rows", new ArrayList<Map<String, Object>>());
			resultMap.put("total", 0);
		}
	...
	public String jsonProblemOrderListInfo(@RequestParam(value = "page", required = false, defaultValue = "1") int page,
			...
			@RequestParam(value = "channelNumber", required = false, defaultValue = "") String channelNumber,
			@RequestParam(value = "sellerId", required = false, defaultValue = "-1") int sellerId,
			@RequestParam(value = "sellerType", required = false, defaultValue = "0") int sellerType) throws Exception {
		Map<String, Object> para = new HashMap<String, Object>();
		if (page == 0) {
			page = 1;
		}
	...
	public String jsonOrderLockedInfo(@RequestParam(value = "page", required = false, defaultValue = "1") int page,
			...
			@RequestParam(value = "checkStatus", required = false, defaultValue = "1") int checkStatus, // 锁定状态
	...
	public String checkOrder(@RequestParam(value = "id", required = false, defaultValue = "0") int id, // 订单id
			...
			@RequestParam(value = "remark", required = false, defaultValue = "") String remark) throws Exception {
		try {
