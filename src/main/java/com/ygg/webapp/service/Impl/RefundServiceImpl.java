...
import com.ygg.webapp.service.RefundService;
...
import com.ygg.webapp.util.OrderEnum.ORDER_STATUS;
import com.ygg.webapp.util.RefundEnum;
...
public class RefundServiceImpl implements RefundService
{
            ...
            throws Exception
    {
        ...
        if (findAllOrderIdsByUserInfo)
        {
            ...
            if (orderIds.size() < 1)
            {
                result.put("rows", new ArrayList<Integer>());
                result.put("total", 0);
                return result;
            }
        }
        ...
        if (refundList.size() > 0)
        {
            ...
            for (RefundEntity refund : refundList)
            {
                ...
                row.put("createTime", refund.getCreateTime());
                row.put("updateTime", refund.getCheckTime());
                ...
                if (refund.getType() == 2)
                {
                    ...
                    parame.put("refundId", refund.getId());
                    parame.put("step", 5);
                    List<RefundTeackEntity> list=refundDao.findOrderProductRefundTeack(parame);
                    canReceive = 1;
                    // 收货状态
                    if (list.size()>0)
                    {
                ...
                if (st == 3 && refund.getType() == 2)
                {
                    ...
                    List<RefundTeackEntity> reList = refundDao.findOrderProductRefundTeack(teackPara);
                    if (reList.size() > 0)
                    {
                        moneyTip = "打款";
                    }
            ...
            throws Exception
    {
        ...
        List<RefundTeackEntity> teackList = refundDao.findOrderProductRefundTeack(para);
        for (RefundTeackEntity teack : teackList)
        {
            ...
            throws Exception
    {
        ...
        // 插入 更新 记录
        RefundTeackEntity teack = new RefundTeackEntity();
            ...
            throws Exception
    {
        ...
        if (refund.getStatus() == CommonEnum.RefundStatusEnum.WAIT_RETURN_OF_GOODS.ordinal() || refund.getStatus() == CommonEnum.RefundStatusEnum.WAIT_RETURN_OF_MONEY.ordinal())
        {
            ...
            if (status == 1)
            {
                ...
                List<RefundTeackEntity> teas = refundDao.findOrderProductRefundTeack(teackPara);
                RefundTeackEntity te = teas.get(0);
                te.setContent(money + "");
    ...
    @Override
    public Map<String, Object> confirmGoods(int id,String remark)
            throws Exception
    {
        RefundEntity refund = refundDao.findRefundById(id);
        ...
        Map<String, Object> para = new HashMap<String, Object>();
        para.put("refundId", refund.getId());
        para.put("step", 5);
        List<RefundTeackEntity> list=refundDao.findOrderProductRefundTeack(para);
        int status=0;
        if(list.size()<1){
        	RefundTeackEntity teack = new RefundTeackEntity();
            teack.setOrderProductRefundId(refund.getId());
            ...
            status=refundDao.saveOrderProductRefundTeack(teack);
    ...
    @Override
    public RefundTeackEntity findRefundTeackByRefundIdAndStep(int refundId, int step)
            throws Exception
    {
        Map<String, Object> para = new HashMap<String, Object>();
        para.put("refundId", refundId);
        para.put("step", step);
        List<RefundTeackEntity> reList = refundDao.findOrderProductRefundTeack(para);
        if (reList.size() > 0)
        {
            return reList.get(0);
        }
